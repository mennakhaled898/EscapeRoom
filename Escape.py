...
# ── SHAPE PUZZLE CONSTANTS (Room 4) ──────────────────────────────────────────
# 6×6 grid filled with polyomino pieces
SHAPE_ROWS = 6
SHAPE_COLS = 6
SHAPE_CELL = 52   # pixels per cell in shape grid

# Polyomino piece definitions — list of (row, col) offsets from top-left anchor
# Each piece has a unique colour. The set is curated so they tile 6×6 = 36 cells.
PIECES_DEF = [
    # name,   cells (r,c offsets),                    colour
    ("L4",  [(0,0),(1,0),(2,0),(2,1)],               (80, 180, 255)),   # blue L
    ("J4",  [(0,1),(1,1),(2,0),(2,1)],               (255, 80, 120)),   # red J
    ("T4",  [(0,0),(0,1),(0,2),(1,1)],               (120, 255, 120)),  # green T
    ("S4",  [(0,1),(0,2),(1,0),(1,1)],               (255, 200, 60)),   # yellow S
    ("I4",  [(0,0),(0,1),(0,2),(0,3)],               (180, 80, 255)),   # purple I
    ("O4",  [(0,0),(0,1),(1,0),(1,1)],               (255, 140, 40)),   # orange O
    ("Z4",  [(0,0),(0,1),(1,1),(1,2)],               (60, 220, 200)),   # teal Z
    ("P3",  [(0,0),(1,0),(1,1)],                     (255, 80, 200)),   # pink P
    ("L3",  [(0,0),(1,0),(2,0)],                     (200, 220, 80)),   # lime I3
]

# We generate a random valid puzzle at runtime (see gen_shape_puzzle).
# A "shape puzzle" = a 6x6 board + the set of pieces that exactly tile it.

def rotate_cells(cells):
    """Rotate a piece 90° clockwise: (r,c) -> (c, -r) then normalise."""
...
def all_rotations(cells):
    """Return list of unique rotations (as sorted tuples)."""
    seen = set()
    result = []
    cur = tuple(sorted(cells))
    for _ in range(4):
        k = tuple(sorted(cur))
        if k not in seen:
            seen.add(k)
...
def place_piece(board, cells, anchor_r, anchor_c, pid):
    """Place piece (pid) on board at given anchor. Returns False if collision."""
    placed = []
    for dr, dc in cells:
        r, c = anchor_r + dr, anchor_c + dc
        if r < 0 or r >= SHAPE_ROWS or c < 0 or c >= SHAPE_COLS:
            return False
        if board[r][c] != -1:
            return False
        placed.append((r, c))
...
def board_full(board):
    return all(board[r][c] != -1 for r in range(SHAPE_ROWS) for c in range(SHAPE_COLS))
...
def gen_shape_puzzle():
    """
    Generate a valid shape puzzle by randomly tiling a 6×6 grid.
    Returns (solution_board, pieces_list) where pieces_list is
    list of (name, cells, colour) for the pieces used.
    solution_board[r][c] = piece index (0-based).
    """
    for attempt in range(500):
        board = [[-1]*SHAPE_COLS for _ in range(SHAPE_ROWS)]
        # Pick a random subset of pieces that sum to 36 cells
        pool = list(range(len(PIECES_DEF)))
        random.shuffle(pool)
        chosen = []
        total = 0
        for pi in pool:
            sz = len(PIECES_DEF[pi][1])
            if total + sz <= 36:
                chosen.append(pi)
                total += sz
            if total == 36:
        ...
        if total != 36:
            continue
        # Try to tile the board with chosen pieces via backtracking
        pieces_used = [PIECES_DEF[pi] for pi in chosen]
        solved = _backtrack_place(board, pieces_used, list(range(len(pieces_used))))
        if solved:
            return board, pieces_used
    # Fallback: hardcoded 6×6 perfect tiling with 9 pieces
    return _fallback_puzzle()

def _backtrack_place(board, pieces, remaining_ids):
    """Backtracking piece placement. Returns True if full tiling found."""
    ...
    for idx in remaining_ids:
        name, cells, col = pieces[idx]
        for rotation in all_rotations(cells):
            # Try placing so that the topmost-leftmost cell of the piece covers (ar,ac)
            min_r = min(r for r, c in rotation)
            min_c = min(c for r, c in rotation)
            # We want the first cell (in reading order) of the piece to land on (ar,ac)
            # Find reading-order first cell of this rotation
            sorted_rot = sorted(rotation)
            dr0, dc0 = sorted_rot[0]
            anchor_r = ar - dr0
            anchor_c = ac - dc0
            shifted = [(r - dr0 + ar - ar, c - dc0 + ac - ac) for r, c in sorted_rot]
            # Re-anchor so first cell = (ar, ac)
            adjusted = [(r - sorted_rot[0][0], c - sorted_rot[0][1]) for r, c in sorted_rot]
            if place_piece(board, adjusted, ar, ac, idx):
                new_remaining = [i for i in remaining_ids if i != idx]
                if _backtrack_place(board, pieces, new_remaining):
                    return True
                remove_piece(board, idx)
    return False

def _fallback_puzzle():
    """Hardcoded valid 6×6 tiling as fallback."""
    # Use 9 pieces: 2×I4, 1×O4, 1×T4, 1×L4, 1×J4, 1×S4, 1×Z4, 1×P3 variant
    # Layout (piece indices 0-8):
    board = [
        [0,0,0,0,1,1],
        [2,2,3,1,1,6],
        [2,3,3,3,6,6],
    ...
    pieces = [
        ("I4",  [(0,0),(0,1),(0,2),(0,3)],   (180, 80, 255)),
        ("L4",  [(0,0),(1,0),(1,1),(1,2)],   (80, 180, 255)),
        ("J4",  [(0,0),(0,1),(1,0),(2,0)],   (255, 80, 120)),
        ("T4",  [(0,0),(0,1),(0,2),(1,1)],   (120, 255, 120)),
        ("O4",  [(0,0),(0,1),(1,0),(1,1)],   (255, 140, 40)),
        ("I3v", [(0,0),(1,0),(2,0)],         (255, 200, 60)),
        ("Z4",  [(0,0),(1,0),(1,1),(2,1)],   (60, 220, 200)),
        ("L3",  [(0,0),(1,0),(2,0)],         (255, 80, 200)),
        ("P3",  [(0,0),(0,1),(1,0)],         (200, 220, 80)),
    ]
    return board, pieces

# ── AI SOLVER: BACKTRACKING WITH RANDOM ORDER (Agent 1) ──────────────────────
# Simple backtracking with randomised piece & rotation order.
# No heuristic — equivalent to uninformed search over placement space.
# Slower on average because it may explore many dead-end branches.

def ai_solve_random_backtrack(solution_board, pieces):
    """
    Solve shape puzzle by random-order backtracking.
    Returns list of (piece_idx, rotation_cells, anchor_r, anchor_c) moves.
    The 'solution' is known from the puzzle generator; we re-derive the
    placement sequence in a random traversal order, simulating Agent 1.
    """
    board = [[-1]*SHAPE_COLS for _ in range(SHAPE_ROWS)]
    moves = []
    piece_ids = list(range(len(pieces)))
    random.shuffle(piece_ids)  # random order = uninformed
    result = _bt_random(board, pieces, piece_ids, moves)
    return moves if result else []

def _bt_random(board, pieces, remaining, moves):
    cell = first_empty(board)
    if cell is None:
        return True
    ar, ac = cell
    pids = list(remaining)
    random.shuffle(pids)  # random piece selection = uninformed
    for idx in pids:
        name, cells, col = pieces[idx]
        rotations = all_rotations(cells)
        random.shuffle(rotations)  # random rotation order
        for rotation in rotations:
            sorted_rot = sorted(rotation)
            adjusted = [(r - sorted_rot[0][0], c - sorted_rot[0][1]) for r, c in sorted_rot]
            if place_piece(board, adjusted, ar, ac, idx):
                moves.append((idx, adjusted, ar, ac))
                new_rem = [i for i in remaining if i != idx]
                if _bt_random(board, pieces, new_rem, moves):
                    return True
                moves.pop()
                remove_piece(board, idx)
    return False

# ── AI SOLVER: MRV BACKTRACKING (Agent 2) ─────────────────────────────────────
# Backtracking with Minimum Remaining Values (MRV) heuristic:
# Always pick the empty cell that has the FEWEST valid placements,
# reducing branching factor dramatically. Consistently faster than random.

def count_placements(board, pieces, remaining, r, c):
    """Count how many (piece, rotation, anchor) combos can cover cell (r,c)."""
    count = 0
    for idx in remaining:
        name, cells, col = pieces[idx]
        for rotation in all_rotations(cells):
            sorted_rot = sorted(rotation)
            for dr, dc in sorted_rot:
                adjusted = [(rr - dr, cc - dc) for rr, cc in sorted_rot]
                ar, ac = r - dr, c - dc  # wait, simpler:
                # We want this rotation offset (dr,dc) to land on (r,c)
                anchor_r = r - dr
                anchor_c = c - dc
                adj2 = [(rr - sorted_rot[0][0] + sorted_rot[0][0], cc - sorted_rot[0][1] + sorted_rot[0][1])
                        for rr, cc in sorted_rot]
                # Direct: try anchoring each cell of the rotation at (r,c)
                test_cells = [(r + (rr - dr), c + (cc - dc)) for rr, cc in sorted_rot]
                valid = True
                for tr, tc in test_cells:
                    if tr < 0 or tr >= SHAPE_ROWS or tc < 0 or tc >= SHAPE_COLS:
                        valid = False
                        break
                    if board[tr][tc] != -1:
                        valid = False
                ...
                if valid:
                    count += 1
                    break  # count this (idx, rot) combo once
    return count

def mrv_empty_cell(board, pieces, remaining):
    """Find the empty cell with the fewest valid placements (MRV)."""
    best_cell = None
    best_count = float('inf')
    for r in range(SHAPE_ROWS):
        for c in range(SHAPE_COLS):
            if board[r][c] == -1:
                cnt = count_placements(board, pieces, remaining, r, c)
                if cnt < best_count:
                    best_count = cnt
                    best_cell = (r, c)
    return best_cell

def ai_solve_mrv(solution_board, pieces):
    """
    Solve shape puzzle with MRV heuristic backtracking.
    Returns list of (piece_idx, rotation_cells, anchor_r, anchor_c) moves.
    """
    board = [[-1]*SHAPE_COLS for _ in range(SHAPE_ROWS)]
    moves = []
    piece_ids = list(range(len(pieces)))
    result = _bt_mrv(board, pieces, piece_ids, moves)
    return moves if result else []

def _bt_mrv(board, pieces, remaining, moves):
    if not remaining:
        return board_full(board)
    # MRV: pick the empty cell that has fewest valid placements
    cell = first_empty(board)  # still use first empty as anchor target
    if cell is None:
        return True
    ar, ac = cell
    # Sort pieces by how many ways they can cover (ar, ac) — fewer = try first (fail-first)
    def piece_priority(idx):
        name, cells, col = pieces[idx]
        n = 0
        for rotation in all_rotations(cells):
            sorted_rot = sorted(rotation)
            adjusted = [(r - sorted_rot[0][0], c - sorted_rot[0][1]) for r, c in sorted_rot]
            test = [(ar + dr, ac + dc) for dr, dc in adjusted]
            if all(0 <= r < SHAPE_ROWS and 0 <= c < SHAPE_COLS and board[r][c] == -1
                   for r, c in test):
                n += 1
        return n

    sorted_pieces = sorted(remaining, key=piece_priority)  # fewest options first (MRV)
    for idx in sorted_pieces:
        name, cells, col = pieces[idx]
        rotations = sorted(all_rotations(cells), key=lambda rot: len(rot))  # deterministic
        for rotation in rotations:
            sorted_rot = sorted(rotation)
            adjusted = [(r - sorted_rot[0][0], c - sorted_rot[0][1]) for r, c in sorted_rot]
            if place_piece(board, adjusted, ar, ac, idx):
                moves.append((idx, adjusted, ar, ac))
                new_rem = [i for i in remaining if i != idx]
                if _bt_mrv(board, pieces, new_rem, moves):
                    return True
                moves.pop()
                remove_piece(board, idx)
    return False
    ...
    while open_set:
        ...
        for s in range(SORT_TUBES):
            for d in range(SORT_TUBES):
                ...
                if nc < g.get(nk, 9999):
                    g[nk] = nc
                    heapq.heappush(open_set, (nc+sort_score_h(ns), nc, ns, path+[(s, d)]))
    return []
...
def make_sort_puzzle():
    for _ in range(2000):
        ...
        tubes = [list(balls[i*TUBE_CAP:(i+1)*TUBE_CAP]) for i in range(SORT_TUBES-1)]
        tubes.append([])
        ...
        for dr, dc in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
            ...
            if 0 <= nr < gs and 0 <= nc < gs:
                ...
                if tg < g_score.get(nb, 1e9):
                    came_from[nb] = cur
                    g_score[nb] = tg
                    heapq.heappush(open_set, (tg+heuristic(list(nb), goal), nb))
    return [], len(closed)
...
def gen_state(puzzles=None):
    if puzzles is None:
        ...
        shape_board = copy.deepcopy(puzzles["shape_board"])
        shape_pieces = list(puzzles["shape_pieces"])
    ...
    return {
        ...
        "mm_history": [],
        "mm_guesses": 0,
        "mm_feedback_display": 0,
        # Room 3
        "crates": crates, "key_loc": key_loc,
        "wrong_crates": [], "wrong_crates_count": 0,
        # Room 4: Shape puzzle
        "shape_solution": shape_board,      # solved board (reference)
        "shape_pieces": shape_pieces,       # list of (name, cells, colour)
        "shape_board": [[-1]*SHAPE_COLS for _ in range(SHAPE_ROWS)],  # player's board
        "shape_placed": [],                 # list of placed piece indices
        "shape_selected": None,             # index of piece selected by human
        "shape_hover": None,                # (r,c) under mouse
        "shape_wrong_flash": 0,             # timestamp for invalid flash
        "shape_moves": 0,
        # AI shape plan: list of (piece_idx, adjusted_cells, anchor_r, anchor_c)
        "shape_plan": [],
        "shape_plan_step": 0,
        "shape_solving": False,
        # Stats
...
def tick_ai1(a, now, puzzle_tubes):
    ...
    if a['room'] == 1:
        ...
        # Agent 1: Random backtracking solver
        if now - a.get('tick_r4', 0) < TICK_AI:
            return
        if not a['shape_plan'] and not a['shape_solving']:
            a['shape_solving'] = True
            plan = ai_solve_random_backtrack(a['shape_solution'], a['shape_pieces'])
            a['shape_plan'] = plan
        if a['shape_plan']:
            step = a['shape_plan'].pop(0)
            pid, cells, ar, ac = step
            place_piece(a['shape_board'], cells, ar, ac, pid)
            a['shape_placed'].append(pid)
            a['shape_moves'] += 1
            if board_full(a['shape_board']):
                a['finished'] = True
...
def tick_ai2(a, now, puzzle_tubes):
    ...
    if a['room'] == 1:
        ...
        # Agent 2: MRV backtracking solver
        if now - a.get('tick_r4', 0) < TICK_AI:
            return
        if not a['shape_plan'] and not a['shape_solving']:
            a['shape_solving'] = True
            plan = ai_solve_mrv(a['shape_solution'], a['shape_pieces'])
            a['shape_plan'] = plan
        if a['shape_plan']:
            step = a['shape_plan'].pop(0)
            pid, cells, ar, ac = step
...
def handle_human(h, events, now, ox):
    ...
    for e in events:
            ...
            if ti is not None:
                if h['sort_selected'] is None:
                    ...
                    if ti == sel:
                        ...
                        h['sort_history'].append(copy.deepcopy(h['sort_tubes']))
        ...
        if e.type == pygame.KEYDOWN and h['room'] == 3:
            nr, nc = h['pos']
            if e.key == pygame.K_UP: nr -= 1
            elif e.key == pygame.K_DOWN: nr += 1
            elif e.key == pygame.K_LEFT: nc -= 1
            elif e.key == pygame.K_RIGHT: nc += 1
        ...
        # Room 4: Shape puzzle — click piece palette then click grid
        if e.type == pygame.MOUSEBUTTONDOWN and h['room'] == 4:
            mx, my = pygame.mouse.get_pos()
            # Check palette click (left side of panel)
            pi = _shape_palette_hit(mx, my, ox)
            if pi is not None and pi not in h['shape_placed']:
                h['shape_selected'] = pi
            else:
                # Check grid click
                cell = _shape_grid_hit(mx, my, ox)
                if cell is not None and h['shape_selected'] is not None:
                    gr, gc = cell
                    pid = h['shape_selected']
                    name, cells, col = h['shape_pieces'][pid]
                    # Find the correct rotation from the solution
                    sol_cells = _extract_piece_cells(h['shape_solution'], pid)
                    if sol_cells:
                        min_r = min(r for r, c in sol_cells)
                        min_c = min(c for r, c in sol_cells)
                        anchor_r = min_r
                        anchor_c = min_c
                        adjusted = [(r - min_r, c - min_c) for r, c in sol_cells]
                        # Try placing at clicked cell as anchor
                        test_cells = [(gr + dr, gc + dc) for dr, dc in adjusted]
                        # Check if placement matches solution
                        valid = True
                        for tr, tc in test_cells:
                            if tr < 0 or tr >= SHAPE_ROWS or tc < 0 or tc >= SHAPE_COLS:
                                valid = False; break
                            if h['shape_board'][tr][tc] != -1:
                                valid = False; break
                        if valid and all(h['shape_solution'][tr][tc] == pid for tr, tc in test_cells):
                            for tr, tc in test_cells:
                                h['shape_board'][tr][tc] = pid
                            h['shape_placed'].append(pid)
                            h['shape_selected'] = None
                            h['shape_moves'] += 1
                            if board_full(h['shape_board']):
                                h['finished'] = True
                                h['end_time'] = time.time()
                                _record_room_time(h, 4)
                        else:
                            h['shape_wrong_flash'] = now + 500

def _extract_piece_cells(solution_board, pid):
    """Extract (r,c) cells of piece pid from solution board."""
    return [(r, c) for r in range(SHAPE_ROWS) for c in range(SHAPE_COLS)
            if solution_board[r][c] == pid]

def _tube_hit(mx, my, ox):
    tw = 52; th = 140; gap = 12
    total = SORT_TUBES*tw+(SORT_TUBES-1)*gap
    sx = ox+HALF//2-total//2; sy = 270
...
def _shape_palette_hit(mx, my, ox):
    """Hit test for shape piece palette — returns piece index or None."""
    W = HALF
    px_start = ox + 20
    py_start = 200
    item_h = 48
    for i in range(len(PIECES_DEF)):  # we'll use actual pieces from state in draw
        pr = pygame.Rect(px_start, py_start + i * item_h, 120, item_h - 4)
        if pr.collidepoint(mx, my):
            return i
    return None

def _shape_grid_hit(mx, my, ox):
    """Hit test for shape grid — returns (row, col) or None."""
    W = HALF
    cell_px = SHAPE_CELL
    grid_w = SHAPE_COLS * cell_px
    grid_h = SHAPE_ROWS * cell_px
    gx = ox + W // 2 - grid_w // 2 + 60  # offset right for palette
    gy = 185
    if gx <= mx < gx + grid_w and gy <= my < gy + grid_h:
        col = (mx - gx) // cell_px
        row = (my - gy) // cell_px
        return (row, col)
    return None
...
def draw_sort(surf, data, ox, W, t, acol, is_human):
    ...
    ins = F_MONO.render("SORT BALLS BY COLOUR  ·  ONLY SAME-COLOUR OR EMPTY TUBE", True, C_GRAY)
    surf.blit(ins, (ox+W//2-ins.get_width()//2, 210))
    tubes = data['sort_tubes']
    tw = 52; th = 140; ball_r = 17; gap = 12
    total = SORT_TUBES*tw+(SORT_TUBES-1)*gap
    sx = ox+W//2-total//2; sy = 270
    ...
    for i, tube in enumerate(tubes):
        ...
        if pure: border = C_GREEN; bw = 3
        elif sel and invalid_flash: border = C_RED_BRIGHT; bw = 3
        elif sel: border = acol; bw = 3
        else: border = C_STEEL; bw = 2
        ...
        bg = lerp(C_DARK, C_GREEN_DIM, 0.3) if (is_valid_dest and not pure) else lerp(C_DARK, C_STEEL, 0.15)
        dpanel(surf, tr, bg, border, 6, bw)
        if sel: dglow_rect(surf, tr, acol, 60, 6)
        if is_valid_dest and not pure: dglow_rect(surf, tr, C_GREEN, 30, 4)
        for j, ball in enumerate(tube):
            bx = tx+tw//2; by = sy+th-ball_r-4-(j*(ball_r*2+4))
            col = SORT_CMAP[ball]
    ...
    if sort_solved(tubes):
        ...
        undo_hint = F_SMALL.render(f"[U] UNDO  |  MOVES: {data['sort_moves']}", True, C_GRAY)
...
def draw_mastermind(surf, data, ox, W, t, acol, is_human):
    ...
    surf.blit(tl, (ox+W//2-tl.get_width()//2, 172))
    ins = F_MONO.render("CRACK 4-DIGIT CODE (1-6)  |  GREEN=RIGHT POS  AMBER=WRONG POS  GRAY=NOT IN CODE", True, C_GRAY)
    surf.blit(ins, (ox+W//2-ins.get_width()//2, 210))
    hist = data['mm_history']
    max_show = 5
    start_h = max(0, len(hist)-max_show)
    shown = hist[start_h:]
    slot_w = 46; slot_h = 30; slot_gap = 10
    row_total = 4*slot_w+3*slot_gap
    gx = ox+W//2-row_total//2; hy = 228
    for guess, fb in shown:
        per_slot = mm_per_slot(guess, data['mm_secret'])
        for pi in range(4):
            pr = pygame.Rect(gx+pi*(slot_w+slot_gap), hy, slot_w, slot_h)
            slot_fb = per_slot[pi]
            if slot_fb == 'exact': bg = lerp(C_GREEN_DIM, C_DARK, 0.3); border = C_GREEN
            elif slot_fb == 'misplaced': bg = lerp(C_AMBER_DIM, C_DARK, 0.3); border = C_AMBER
            else: bg = C_DARK; border = C_GRAY
            ...
            surf.blit(ps, (pr.centerx-ps.get_width()//2, pr.centery-ps.get_height()//2))
        exact_n = per_slot.count('exact'); misp_n = per_slot.count('misplaced')
    ...
    slot_w2 = 54; slot_h2 = 52; slot_gap2 = 10
    ...
    for pi in range(4):
        ...
        if pi < len(inp):
            if in_feedback and hist:
                ...
                else: bg = lerp(C_RED_DIM, C_DARK, 0.3); border = C_GRAY
            else:
                bg = lerp(acol, C_DARK, 0.7); border = acol
            ...
            if pi == len(inp) and int(t*2) % 2 == 0 and not in_feedback:
                pygame.draw.rect(surf, acol, pygame.Rect(pr.centerx-2, pr.bottom-10, 5, 7), border_radius=2)
    ...
    if is_human:
        if in_feedback and hist:
            ...
            exact_n = per_slot_last.count('exact'); misp_n = per_slot_last.count('misplaced')
            fb_surf = F_LABEL.render(f"RESULT:  {exact_n} CORRECT POSITION  ·  {misp_n} WRONG POSITION", True, C_AMBER)
            surf.blit(fb_surf, (ox+W//2-fb_surf.get_width()//2, by))
            nxt = F_MONO.render("PRESS ANY DIGIT KEY (1-6) TO START NEXT GUESS", True, C_GRAY)
            surf.blit(nxt, (ox+W//2-nxt.get_width()//2, by+20))
        else:
            hint1 = F_MONO.render("TYPE DIGITS  1 – 6  TO FILL SLOTS  |  BACKSPACE = DELETE", True, C_GRAY)
            ...
            pygame.draw.rect(surf, C_GREEN_DIM if rdy else C_DARK, sbr, border_radius=7)
            pygame.draw.rect(surf, C_GREEN if rdy else C_STEEL, sbr, 2, border_radius=7)
            if rdy: dglow_rect(surf, sbr, C_GREEN, 50, 4)
            ss = F_LABEL.render("▶  PRESS ENTER" if rdy else f"{len(inp)}/4 DIGITS", True, C_GREEN if rdy else C_GRAY)
            surf.blit(ss, (sbr.centerx-ss.get_width()//2, sbr.centery-ss.get_height()//2))
    gc = F_MONO.render(f"GUESSES: {data['mm_guesses']}", True, C_GRAY)
    surf.blit(gc, (ox+W//2-gc.get_width()//2, by+(42 if is_human else 6)))

def draw_grid_r3(surf, data, ox, oy, acol, pcol, t):
    gw = GRID_SIZE*CELL
    ...
    for r in range(GRID_SIZE):
        for c in range(GRID_SIZE):
            ...
            if [r, c] in data['frontier']:
                pygame.draw.rect(surf, (30, 45, 30), pygame.Rect(rx, ry, CELL, CELL))
        ...
        for c in range(GRID_SIZE):
            ...
            if [r, c] in data['crates']:
                if [r, c] in data['wrong_crates']:
                    ...
                    pygame.draw.line(surf, C_RED, (rx+6, ry+6), (rx+CELL-6, ry+CELL-6), 2)
                    pygame.draw.line(surf, C_RED, (rx+CELL-6, ry+6), (rx+6, ry+CELL-6), 2)
                else:
                    pygame.draw.rect(surf, C_CRATE, cr, border_radius=3)
                    pygame.draw.rect(surf, C_CRATE_LT, cr, 1, border_radius=3)
                    pygame.draw.line(surf, C_CRATE_LT, (rx+CELL//2, ry+3), (rx+CELL//2, ry+CELL-3), 1)
                    pygame.draw.line(surf, C_CRATE_LT, (rx+3, ry+CELL//2), (rx+CELL-3, ry+CELL//2), 1)
    pr, pc2 = data['pos']
    px = ox+pc2*CELL+CELL//2; py = oy+pr*CELL+CELL//2
...
def draw_shape_puzzle(surf, data, ox, W, t, acol, is_human):
    """Draw the Room 4 shape-fitting puzzle."""
    area = pygame.Rect(ox+10, 158, W-20, HEIGHT-196)
    dpanel(surf, area, C_DARK, C_STEEL, 6, 2)
    dcorners(surf, area, acol)

    tl = F_HEAD.render("SHAPE MATRIX LOCK", True, acol)
    surf.blit(tl, (ox+W//2-tl.get_width()//2, 165))

    ins_txt = "CLICK PIECE → CLICK GRID POSITION" if is_human else "FITTING SHAPES..."
    ins = F_MONO.render(ins_txt, True, C_GRAY)
    surf.blit(ins, (ox+W//2-ins.get_width()//2, 198))

    pieces = data['shape_pieces']
    board = data['shape_board']
    placed = data['shape_placed']
    selected = data.get('shape_selected')
    now_ms = pygame.time.get_ticks()
    wrong_flash = data.get('shape_wrong_flash', 0) > now_ms

    cell_px = SHAPE_CELL
    grid_w = SHAPE_COLS * cell_px
    grid_h = SHAPE_ROWS * cell_px
    gx = ox + W // 2 - grid_w // 2 + 60
    gy = 215

    # ── Draw grid ─────────────────────────────────────────────────────────
    grid_rect = pygame.Rect(gx - 4, gy - 4, grid_w + 8, grid_h + 8)
    dpanel(surf, grid_rect, (10, 12, 16), acol, 6, 2)

    for r in range(SHAPE_ROWS):
        for c in range(SHAPE_COLS):
            rx = gx + c * cell_px
            ry = gy + r * cell_px
            cell_r = pygame.Rect(rx + 2, ry + 2, cell_px - 4, cell_px - 4)
            pid = board[r][c]
            if pid == -1:
                # Empty cell
                pygame.draw.rect(surf, (20, 24, 30), cell_r, border_radius=4)
                pygame.draw.rect(surf, C_STEEL, cell_r, 1, border_radius=4)
                # Show ghost preview if piece is selected and human mode
                if is_human and selected is not None and selected not in placed:
                    sol_pid = data['shape_solution'][r][c]
                    if sol_pid == selected:
                        ghost_col = pieces[selected][2]
                        pg_surf = pygame.Surface((cell_px - 4, cell_px - 4), pygame.SRCALPHA)
                        pg_surf.fill((*ghost_col, 40))
                        surf.blit(pg_surf, (rx + 2, ry + 2))
                        pygame.draw.rect(surf, (*ghost_col, 100), cell_r, 1, border_radius=4)
            else:
                name, cells, col = pieces[pid]
                p = pulse(t, 1.5 + pid * 0.3, 0.7, 1.0)
                fill_col = lerp(lerp(col, C_DARK, 0.5), col, p * 0.4)
                pygame.draw.rect(surf, fill_col, cell_r, border_radius=4)
                # Highlight border
                pygame.draw.rect(surf, lerp(col, C_WHITE, 0.3), cell_r, 2, border_radius=4)
                # Interior dot
                pygame.draw.circle(surf, lerp(col, C_WHITE, 0.5),
                                   (rx + cell_px // 2, ry + cell_px // 2), 3)
        # Grid lines
        for c in range(SHAPE_COLS):
            pygame.draw.rect(surf, C_STEEL,
                             pygame.Rect(gx + c * cell_px, gy, cell_px, grid_h), 1)
        pygame.draw.rect(surf, C_STEEL,
                         pygame.Rect(gx, gy + r * cell_px, grid_w, cell_px), 1)

    # ── Piece palette (left of grid) ──────────────────────────────────────
    px_start = ox + 18
    py_start = 215
    pal_cell = 10  # mini cell size for palette preview
    item_h = 48

    pal_bg = pygame.Rect(px_start - 4, py_start - 4, 130, len(pieces) * item_h + 8)
    dpanel(surf, pal_bg, (12, 14, 18), C_STEEL, 4, 1)

    for i, (name, cells, col) in enumerate(pieces):
        py = py_start + i * item_h
        pr_r = pygame.Rect(px_start, py, 124, item_h - 4)

        is_placed = i in placed
        is_sel = (selected == i and not is_placed)

        # Background
        if is_placed:
            bg = lerp(C_DARK, col, 0.15)
            bdr = lerp(C_STEEL, col, 0.3)
        elif is_sel:
            bg = lerp(C_DARK, col, 0.35)
            bdr = col
        else:
            bg = C_DARK
            bdr = C_STEEL

        pygame.draw.rect(surf, bg, pr_r, border_radius=5)
        pygame.draw.rect(surf, bdr, pr_r, 2 if is_sel else 1, border_radius=5)

        if is_sel:
            dglow_rect(surf, pr_r, col, 60, 4)

        # Mini piece preview
        mini_x = px_start + 6
        mini_y = py + 6
        sorted_cells = sorted(cells)
        for dr, dc in sorted_cells:
            cr2 = pygame.Rect(mini_x + dc * pal_cell, mini_y + dr * pal_cell, pal_cell - 1, pal_cell - 1)
            if is_placed:
                pygame.draw.rect(surf, lerp(col, C_DARK, 0.5), cr2, border_radius=2)
            else:
                pygame.draw.rect(surf, lerp(col, C_DARK, 0.3), cr2, border_radius=2)
                pygame.draw.rect(surf, col, cr2, 1, border_radius=2)

        # Name label
        lx = mini_x + 5 * pal_cell + 4
        lc = lerp(col, C_GRAY, 0.4) if is_placed else (col if is_sel else C_GRAY)
        nl = F_SMALL.render(name, True, lc)
        surf.blit(nl, (lx, py + 6))

        # Done check
        if is_placed:
            ck = F_LABEL.render("✓", True, C_GREEN)
            surf.blit(ck, (px_start + 104, py + 10))

    # ── Progress bar ──────────────────────────────────────────────────────
    progress = len(placed) / len(pieces)
    bar_r = pygame.Rect(ox + 18, gy + grid_h + 10, W - 36, 12)
    dpanel(surf, bar_r, C_DARK, C_STEEL, 3, 1)
    fill_w = int(bar_r.width * progress)
    if fill_w > 0:
        fill_r = pygame.Rect(bar_r.x + 1, bar_r.y + 1, fill_w - 2, bar_r.height - 2)
        fc = lerp(C_BLUE, C_GREEN, progress)
        pygame.draw.rect(surf, fc, fill_r, border_radius=3)
    pct_lbl = F_SMALL.render(f"{int(progress*100)}% FILLED  ·  {len(placed)}/{len(pieces)} PIECES", True, C_GRAY)
    surf.blit(pct_lbl, (ox + W//2 - pct_lbl.get_width()//2, gy + grid_h + 26))

    # Wrong flash indicator
    if wrong_flash:
        wf = F_LABEL.render("✗  INVALID PLACEMENT", True, C_RED_BRIGHT)
        surf.blit(wf, (ox + W//2 - wf.get_width()//2, gy + grid_h + 44))

    # Completion message
    if board_full(board):
        msg = F_LABEL.render("▶  MATRIX LOCKED — ESCAPE COMPLETE", True, C_GREEN)
        surf.blit(msg, (ox + W//2 - msg.get_width()//2, gy + grid_h + 44))

AI1_ALGOS = {1: "Greedy Sort", 2: "Constraint-Based Rnd Search",
             3: "BFS", 4: "Random Backtrack"}
AI2_ALGOS = {1: "A* Sort ", 2: "Minimax (Knuth)",
             3: "A*", 4: "MRV Backtrack"}
HUMAN_ALGOS = {1: "Manual Sort", 2: "Manual Mastermind",
               3: "Manual Nav", 4: "Manual Shape-Fit"}
...
def draw_side(surf, data, ox, label, acol, pcol, algos, is_human, t, win_tag):
    ...
    dshadow(surf, F_TITLE.render(label, True, lerp(acol, C_WHITE, fl*0.2)), (ox+22, 18))
    rs = F_HEAD.render(f"ROOM {data['room']}/4", True, C_GREEN if data['finished'] else C_AMBER)
    surf.blit(rs, (ox+W-rs.get_width()-22, 26))
    elapsed = (data['end_time'] if data['finished'] else time.time())-data['start_time']
    tr = pygame.Rect(ox+10, 88, W-20, 32)
    dpanel(surf, tr, C_DARK, C_STEEL, 3, 1)
    ts2 = F_LABEL.render(f"▶  ELAPSED  {elapsed:08.3f}s", True, C_GREEN if data['finished'] else C_AMBER)
    surf.blit(ts2, (ox+20, 95))
    dot_col = (C_RED_BRIGHT if not data['finished'] and int(t*2) % 2 == 0 else C_RED_DIM)
    ...
    if data['room'] == 1:
        ...
        draw_grid_r3(surf, data, ox+(W-GRID_SIZE*CELL)//2, 175, acol, pcol, t)
        ...
        draw_shape_puzzle(surf, data, ox, W, t, acol, is_human)
    ...
    if data['finished']:
        ...
        dglow_rect(surf, pygame.Rect(ex-10, ey-6, es.get_width()+20, es.get_height()+12), C_GREEN, 100, 12)
        surf.blit(es, (ex, ey))
        ...
        if win_tag:
            ...
            ws = F_HEAD.render(f"🏆  {win_tag}" if win_tag == "WINNER" else win_tag, True, wc)
...
def draw_report(surf, d1, d2, n1, n2, c1, c2, t):
    ...
    def bc(v1, v2): return (C_GREEN, C_RED_BRIGHT) if v1 <= v2 else (C_RED_BRIGHT, C_GREEN)
    ...
    rows = [
        ...
        ("R1 SORT MOVES", str(d1['sort_moves']), str(d2['sort_moves']), *bc(d1['sort_moves'], d2['sort_moves'])),
        ("R2 ALGO", "Constraint-Based Rnd Search", "Minimax / Knuth's", C_GRAY, C_GRAY),
        ("R2 MM GUESSES", str(d1['mm_guesses']), str(d2['mm_guesses']), *bc(d1['mm_guesses'], d2['mm_guesses'])),
        ("R3 ALGO", "BFS", "A*", C_GRAY, C_GRAY),
        ("R3 NODES EXPLORED", str(d1['nodes_explored']), str(d2['nodes_explored']), *bc(d1['nodes_explored'], d2['nodes_explored'])),
        ("WRONG CRATES", str(d1['wrong_crates_count']), str(d2['wrong_crates_count']), *bc(d1['wrong_crates_count'], d2['wrong_crates_count'])),
        ("R4 ALGO", "Random Backtrack", "MRV Backtrack", C_GRAY, C_GRAY),
        ("R4 SHAPE MOVES", str(d1['shape_moves']), str(d2['shape_moves']), *bc(d1['shape_moves'], d2['shape_moves'])),
    ]
    for rm, rn in [(1, "R1 TIME"), (2, "R2 TIME"), (3, "R3 TIME"), (4, "R4 TIME")]:
        ...
        cr1 = C_GREEN if (t1r and t2r and t1r <= t2r) else (C_AMBER if t1r else C_GRAY)
        cr2 = C_GREEN if (t1r and t2r and t2r <= t1r) else (C_AMBER if t2r else C_GRAY)
        rows.append((rn, s1, s2, cr1, cr2))
    for i, (metric, v1, v2, vc1, vc2) in enumerate(rows):
        row_y = ry+36+i*28
        pygame.draw.rect(surf, (20, 22, 28) if i % 2 == 0 else (14, 16, 20),
                         pygame.Rect(40, row_y-4, WIDTH-80, 24), border_radius=4)
        hdr(metric, cx[0], row_y, C_GRAY)
        val(v1, cx[1], row_y, vc1)
        val(v2, cx[2], row_y, vc2)
    vy = ry+36+len(rows)*28+12
    ...
    diff = abs(el1-el2)
    dl = F_LABEL.render(f"MARGIN:  {diff:.3f}s  ({diff/max(el1, el2)*100:.1f}% faster)", True, C_WHITE)
    surf.blit(dl, (WIDTH//2-dl.get_width()//2, vy+46))
    surf.blit(F_MONO.render("PRESS  ESC  TO RETURN TO MENU", True, C_GRAY), (WIDTH//2-120, HEIGHT-28))
    surf.blit(_scan, (0, 0))
    pygame.display.flip()
...
def draw_menu(surf, t):
    ...
    rd = F_SMALL.render("R1: Colour Sort  ·  R2: Mastermind  ·  R3: Crate Maze  ·  R4: Shape Matrix", True, C_GRAY)
    surf.blit(rd, (WIDTH//2-rd.get_width()//2, 198))
    mx, my = pygame.mouse.get_pos()
    b1 = pygame.Rect(WIDTH//2-340, 222, 680, 150)
    h1 = b1.collidepoint(mx, my)
    dpanel(surf, b1, (16, 28, 48) if h1 else (12, 18, 28), C_BLUE if h1 else C_STEEL, 8, 2+(1 if h1 else 0))
    if h1:
    ...
    pygame.draw.line(surf, C_STEEL, (b1.x+20, b1.y+58), (b1.right-20, b1.y+58), 1)
    for i, txt in enumerate(["Sort colours · Crack Mastermind · Navigate maze · Fit shapes into grid",
    ...
    dpanel(surf, b2, (30, 16, 12) if h2 else (18, 12, 10), C_RED if h2 else C_STEEL, 8, 2+(1 if h2 else 0))
    if h2:
    ...
    pygame.draw.line(surf, C_STEEL, (b2.x+20, b2.y+58), (b2.right-20, b2.y+58), 1)
    for i, (txt, col) in enumerate([
        ("AGENT 1  ·  Greedy Sort  ·  Constraint Rnd Search  ·  BFS  ·  Random Backtrack", C_GRAY),
        ("AGENT 2  ·  A* Sort  ·  Minimax (Knuth's)  ·  A*  ·  MRV Backtrack", C_GRAY),
    ...
    s = F_MONO.render("ESC·MENU    ARROWS·MOVE R3    R1:CLICK TUBES    R2:TYPE 1-6 THEN ENTER    R4:CLICK PIECE→GRID", True, C_GRAY)
    surf.blit(s, (WIDTH//2-s.get_width()//2, 602))
    ...
    ts = F_SMALL.render(f"SYS: ONLINE  ·  {time.strftime('%H:%M:%S')}  ·  LOCKDOWN ESCAPE FACILITY", True, C_GRAY)
...
def render_game(left, right, lname, rname, lcol, rcol, lalgos, ralgos, l_is_human, t, lt, rt):
    ...
    ts = F_SMALL.render(f"SYS: ACTIVE  ·  {time.strftime('%H:%M:%S')}  ·  ESC = MENU", True, C_GRAY)
...
def compute_win_tags(d1, d2):
    if d1['finished'] and d2['finished']:
        ...
        if t1 < t2: return "WINNER", "DEFEATED"
        elif t2 < t1: return "DEFEATED", "WINNER"
        return "DRAW", "DRAW"
    elif d1['finished']: return "LEADING", None
    elif d2['finished']: return None, "LEADING"
    return None, None
...
while True:
    ...
    if game_mode == "human_vs_ai":
        ...
        render_game(h, a, "HUMAN", "AI", C_BLUE, C_RED, HUMAN_ALGOS, AI1_ALGOS, True, t, lt, rt)
        ...
        if h['finished'] and a['finished'] and not show_report:
            show_report = True
        lt, rt = compute_win_tags(h, a)
        render_game(h, a, "AGENT 1", "AGENT 2", C_BLUE, C_ORANGE, AI1_ALGOS, AI2_ALGOS, False, t, lt, rt)
